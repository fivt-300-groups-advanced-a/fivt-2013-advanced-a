//јсимптотика O(N log N log L) N = количество, L = средн€€ длина строк
...
const int MAX_L = 105; //когда-нибудь € заменю это на динамическое выделение пам€ти
...
using namespace std; //да прост€т мен€ боги

//возвращает номер первого символа, в котором строки различаютс€; если строки равны, или одна из них €вл€етс€ подстрокой другой, то возвращает -1
...
MyBlock findBordersOfBlock(int ps, int pch) //pch - символ, который точно есть в блоке, ps - позици€ строки в data
{
	MyBlock ans;
	ans.pstr = ps;
...
//возвращает 1, если строка, оканчивающа€с€ на блок а должна быть раньше, при условии, что префиксы до блоков a и b одинаковы.
bool cmpBlock(MyBlock a, MyBlock b);

//возвращает 1, если строка, лежаща€ в data[ps] должна быть раньше, чем строка лежаща€ в data[pt]
...
int main()
{
	...
	//предподсчЄт множителей дл€ хэшировани€
	...
	//ввод и инициализаци€
	//data после ввода мен€ть низз€
	for (int i = 0; i < n; ++i)
	{
		order[i] = i;
		char ch[MAX_L];
		cin >> ch;
		if (strlen(ch) > 100) //убрат, когда напишешь динамическое выделение пам€ти
			cout << "¬џ¬ќƒ ћќ∆≈“ Ѕџ“№ Ќ≈ ќ––≈ “≈Ќ, ѕќћ≈Ќя…“≈ ¬ќЋЎ≈ЅЌ”ё  ќЌ—“ј“Ќ”" << endl;
