...
#include "SegmentAdditionAssignmentTree.h"

#include "dummystructure.h"

TEST(AdditionAssignment, SmallManual)
{
...
//    std::size_t n = data.size();
    ...
    for (auto test : tests)
    {
        dummy.add(test[0][0], test[0][1], test[0][2]);
        tree.add(test[0][0], test[0][1], test[0][2]);
...
TEST(AdditionAssignment, StressAddAssignGet)
{
    for (int seed = 0; seed < 100; ++seed)
    {
        ...
        SegmentAdditionAssignmentTree<int> tree(init.begin(), init.end(),
        ...
        for (int i = 0; i < 100; ++i)
        {
            ...
            ASSERT_EQ(dummy.sum(l, r), res.sum) << "seed: " << seed << " i: " << i;
            ASSERT_EQ(dummy.min(l, r), res.min) << "seed: " << seed << " i: " << i;
            ASSERT_EQ(dummy.max(l, r), res.max) << "seed: " << seed << " i: " << i;
        }
    }
}
